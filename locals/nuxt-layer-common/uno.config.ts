...
// Shared UnoCSS design system. `@unocss/nuxt` only loads the app's own config file, so each app
// re-exports this from its `uno.config.ts`.
  ...
  content: {
    pipeline: {
      include: [
        ...
        // Layer's shadcn js/ts files. Absolute glob: UnoCSS resolves relative globs
        // against the consuming app, not against this layer.
        `${resolve(layerDir, 'app/lib/shadcn')}/**/*.{js,ts}`,
      ],
    },
  },
})
